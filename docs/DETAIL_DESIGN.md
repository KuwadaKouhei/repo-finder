...
方針: テスティングトロフィー（統合厚め）・振る舞いをテスト・MSWでネットワーク境界をモック。表示コンポーネントは props のみ受け取る純粋な部品にしてテスト容易性を確保。

| テスト（要件 §3.6） | 対象 | 内容 |
| --- | --- | --- |
| アダプタ単体テスト | `adapters.ts` | 変換・`language:null`・`watchers=subscribers_count`・URLスキーム検証・数値欠損 |
| クライアントエラーの写像 | `client.ts` | MSWで403/404/422→型付きエラー |
| 検索フロー結合テスト | 検索画面 | submit→結果 / 0件→空 / 403→エラー / view切替 |
| ソート統合テスト | SortControl + URL | `sort`/`order`反映・URL同期・page リセット |
| 詳細描画テスト | 詳細画面 | 7項目・a11y・外部リンク（`href`/`rel`） |
